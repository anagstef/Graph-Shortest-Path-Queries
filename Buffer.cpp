...
void Buffer::addNewEdgeDirectly(uint32_t to, uint32_t from, uint32_t version, NodeIndex &index){
    int len;
    uint32_t listTail = index.getListTail(from);
    uint32_t new_listnode_id;

    len = nodes[listTail].get_length();

    if(len < N){ //if the capacity is not full add it
        nodes[listTail].set_neighbor(len, to, version);
        ...
        nodes[new_listnode_id].set_neighbor(0, to, version);
        nodes[new_listnode_id].set_length(1);
        index.setListTail(from, new_listnode_id);
    }

    index.addNeighbor(from);
}
